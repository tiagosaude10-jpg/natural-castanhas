const CACHE='natural-castanhas-etapa2-acesso-20260806';
const CORE_FILES=[
  ...
  './auth-login-hotfix.js','./dashboard-consolidado.js','./dashboard-etapa1.js','./recuperacao-admin-etapa2.js','./nucleo-8-transporte.html',
...
const PAGE_SCRIPTS=[
  ...
  'auth-login-hotfix.js','dashboard-consolidado.js','dashboard-etapa1.js','recuperacao-admin-etapa2.js'
];
...
async function homeResponse(request){
  ...
  if(!html.includes('ios-form-fix.css')){
    html=html.replace('</head>','<link rel="stylesheet" href="./ios-form-fix.css?v=39"></head>');
  }
  PAGE_SCRIPTS.forEach(name=>{
    if(!html.includes(name))html=html.replace('</body>',`<script src="./${name}?v=39"></script></body>`);
  });
